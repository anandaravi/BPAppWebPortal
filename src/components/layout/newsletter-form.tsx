...
export function NewsletterForm({ source = "footer" }: { source?: string }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/subscribe", {
        ...
        body: JSON.stringify({ email, source, honeypot: hp }),
      });
  ...
  return (
    <form onSubmit={submit} className="space-y-2">
      <p className="text-xs font-semibold uppercase tracking-widest text-amber-500">Stay updated</p>
      <p className="text-xs text-text-3 leading-relaxed">
        Product updates + paper-industry insights. No spam.
      </p>
      <input
        type="text"
        tabIndex={-1}
      ...
      <div className="flex gap-2">
        ...
        >
          {status === "loading" ? <Loader2 size={12} className="animate-spin" /> : <Mail size={12} />}
          Subscribe
        </button>
      </div>
      {status === "error" && (
