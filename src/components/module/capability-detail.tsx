    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-[var(--container-max)] mx-auto px-6 pt-32 pb-16">
          <nav className="flex items-center gap-2 text-xs text-text-3 mb-8">
            ...
            <ChevronRight size={12} />
            <span className="text-text-2">{capability.title}</span>
          </nav>
          ...
          <div>
            ...
            <p className="text-lg text-text-2 max-w-3xl leading-relaxed mb-10">{blurb}</p>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <Link href={`/product/${module.slug}`} className="inline-flex items-center gap-2 px-6 py-3.5 rounded-lg border border-border-light text-foreground hover:border-amber-500/40 text-sm font-medium transition-all">
      ...
      {/* PROCESS FLOW */}
      <section className="py-16 border-y border-border-dim bg-background">
        <div className="max-w-[var(--container-max)] mx-auto px-6">
          <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: accent }}>Process Flow</p>
          <h2 className="text-3xl font-black text-foreground tracking-tight mb-10">How it works.</h2>

          <div className="bg-surface border border-border-dim rounded-2xl p-6 md:p-8">
              ...
              {flow.map((step, i) => (
                <div key={`${i}-${step.label}`} className="flex items-center gap-3">
                  <div
                    ...
                    <span className="text-[10px] font-mono text-text-3 mb-1">{String(i + 1).padStart(2, "0")}</span>
                    <span className="text-sm font-bold text-foreground">{step.label}</span>
                    {step.sub && <span className="text-[10px] text-text-3 mt-0.5 font-mono">{step.sub}</span>}
                  </div>
                  {i < flow.length - 1 && (
        ...
        <div className="max-w-[var(--container-max)] mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl font-black text-foreground tracking-tight mb-5">What you get.</h2>
            ...
            <div className="space-y-3">
              {features.map((f, i) => (
                <div key={`${i}-${f.slice(0, 40)}`}
                  className="bg-surface border border-border-dim rounded-xl p-4 flex items-start gap-3 hover:border-amber-500/20 transition-colors">
                  <CheckCircle2 size={16} className="mt-0.5 flex-shrink-0" style={{ color: accent }} />
                  <p className="text-sm text-text-2">{f}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="py-20 bg-background border-b border-border-dim">
        <div className="max-w-[var(--container-max)] mx-auto px-6">
          ...
          <h2 className="text-3xl font-black text-foreground tracking-tight mb-10">How it looks.</h2>

          <div className="bg-surface border border-border-dim rounded-2xl overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-3.5 border-b border-border-dim bg-background">
              <div className="flex gap-1.5">
                <span className="w-2.5 h-2.5 rounded-full bg-red-500/70" />
                <span className="w-2.5 h-2.5 rounded-full bg-amber-500/70" />
                <span className="w-2.5 h-2.5 rounded-full bg-emerald-500/70" />
              </div>
              <span className="text-xs text-text-3 ml-2 font-mono">Papyrus BPApp · {capability.title}</span>
              <span className="ml-auto text-xs text-text-4 font-mono">{module.slug}/{slugify(capability.title)}</span>
            </div>
            <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-background rounded-xl border border-border-dim p-4">
                <p className="text-[10px] text-text-3 uppercase tracking-wider mb-2 font-semibold">Status</p>
                <div className="flex items-center gap-2 mb-3">
                  <span className="w-2 h-2 rounded-full" style={{ background: accent }} />
                  <p className="text-sm font-bold text-foreground">Active</p>
                </div>
                <p className="text-[10px] text-text-3">Last activity 2 min ago</p>
              </div>

              <div className="bg-background rounded-xl border border-border-dim p-4">
                <p className="text-[10px] text-text-3 uppercase tracking-wider mb-2 font-semibold">Today</p>
                <p className="text-2xl font-black font-mono text-foreground">142</p>
                <p className="text-[10px] mt-1" style={{ color: accent }}>↑ 12% vs yesterday</p>
              </div>

              <div className="bg-background rounded-xl border border-border-dim p-4">
                <p className="text-[10px] text-text-3 uppercase tracking-wider mb-2 font-semibold">SLA</p>
                <p className="text-2xl font-black font-mono text-foreground">98.7%</p>
            ...
            <div className="px-6 pb-6">
              <div className="bg-background rounded-xl border border-border-dim p-4">
                <p className="text-[10px] text-text-3 uppercase tracking-wider mb-3 font-semibold">Recent Activity</p>
                <div className="space-y-2">
                  {flow.map((step, i) => (
                    <div key={`act-${i}`} className="flex items-center gap-3 py-1.5">
                      ...
                      <span className="text-xs text-text-2 flex-1">{step.label}</span>
                      <span className="text-[10px] text-text-4 font-mono">{step.sub}</span>
                      <span className="text-[10px] text-text-4 font-mono">{i === 0 ? "now" : `${i * 2}m ago`}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {related.length > 0 && (
        <section className="py-20 border-b border-border-dim">
          <div className="max-w-[var(--container-max)] mx-auto px-6">
            ...
            <h2 className="text-3xl font-black text-foreground tracking-tight mb-10">More capabilities.</h2>
                ...
                return (
                  <div key={`rel-${i}-${r.slug}`}>
                    <Link href={`/product/${module.slug}/${r.slug}`}
                      className="block bg-surface border border-border-dim rounded-xl p-4 hover:border-amber-500/30 transition-colors group">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-bold text-foreground truncate group-hover:text-amber-400 transition-colors">{r.title}</p>
                        </div>
      ...
      <section className="py-24 bg-background">
        <div className="max-w-3xl mx-auto px-6 text-center">
          ...
          <h2 className="text-4xl font-black text-foreground tracking-tight mb-4">See {capability.title} in your mill.</h2>
