    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="pt-32 pb-12 px-6 max-w-[var(--container-max)] mx-auto">
        ...
        <motion.p
          ...
          className="text-text-2 text-lg max-w-2xl mb-10">
      ...
      {/* GROUP FILTER PILLS */}
      {!search && (
        <section className="px-6 max-w-[var(--container-max)] mx-auto mb-8">
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-colors ${
                activeGroup === "all"
                  ...
                  : "bg-surface border border-border-dim text-text-2 hover:text-foreground hover:border-amber-500/30"
              }`}>
              All Groups
            </button>
            {MODULE_GROUPS.map((g) => (
      ...
      {/* RESULTS / GROUPS */}
      <section className="px-6 max-w-[var(--container-max)] mx-auto pb-24">
        <AnimatePresence mode="wait">
          {search && filteredCaps ? (
            ...
            <motion.div
              ...
              <p className="text-xs font-semibold uppercase tracking-widest text-text-3 mb-6 font-mono">
                {filteredCaps.length} {filteredCaps.length === 1 ? "result" : "results"} for &ldquo;{search}&rdquo;
              </p>

              {filteredCaps.length === 0 ? (
                <div className="bg-surface border border-border-dim rounded-2xl p-10 text-center">
                  <p className="text-text-3">No capabilities match your search. Try a broader term.</p>
                </div>
              ) : (
                    ...
                    return (
                      <motion.div key={`${cap.module.slug}-${cap.capSlug}`}
                        ...
                        <Link href={`/product/${cap.module.slug}/${cap.capSlug}`}
                          className="block h-full bg-surface border border-border-dim rounded-2xl p-5 hover:border-amber-500/30 hover:bg-[#121212] transition-all group">
                          <div className="flex items-start justify-between mb-3">
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              {cap.status === "roadmap" && (
                                <span className="px-1.5 py-0.5 rounded-full text-[9px] font-semibold uppercase tracking-wider bg-surface-2 text-text-2 border border-border-light">Roadmap</span>
                              )}
                          ...
                          <h3 className="text-foreground font-bold mb-1.5 text-sm leading-tight group-hover:text-amber-400 transition-colors">{cap.title}</h3>
                          <p className="text-text-3 text-[12px] leading-relaxed line-clamp-3">{cap.desc}</p>
                        </Link>
                      </motion.div>
                    );
                  })}
              ...
              {visibleGroups.map((group, gi) => (
                <div key={group.title}>
                  <motion.div
                    ...
                    <h2 className="text-2xl font-black text-foreground">{group.title}</h2>
                    ...
                    {group.slugs.map((slug, mi) => {
                      ...
                      return (
                        <motion.div key={slug}
                          ...
                          className="bg-surface border border-border-dim rounded-2xl overflow-hidden hover:border-amber-500/20 transition-colors">
                          <Link href={`/product/${slug}`} className="block p-5 border-b border-border-dim hover:bg-[#121212] transition-colors group">
                            <div className="flex items-center gap-4">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-0.5">
                                  <h3 className="text-foreground font-bold text-lg">{m.name}</h3>
                                  <span className="text-[10px] font-mono text-text-4">{m.capabilities.length} capabilities</span>
                                </div>
                                <p className="text-sm text-text-3 line-clamp-1">{m.tag}</p>
                              </div>
                            ...
                            {m.capabilities.map((cap, ci) => {
                              ...
                              return (
                                <Link key={cap.title || ci}
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-1.5">
                                      <p className="text-xs font-semibold text-foreground group-hover:text-amber-400 transition-colors leading-tight truncate">{cap.title}</p>
                                      {cap.status === "preview" && (
                                      ...
                                      {cap.status === "roadmap" && (
                                        <span className="flex-shrink-0 px-1 py-0 rounded text-[8px] font-bold uppercase bg-surface-2 text-text-3 border border-border-light">Roadmap</span>
                                      )}
                                    </div>
                                    <p className="text-[10px] text-text-4 line-clamp-1 mt-0.5">{cap.desc}</p>
                                  </div>
