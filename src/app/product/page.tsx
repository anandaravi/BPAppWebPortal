    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="pt-32 pb-12 px-6 max-w-[var(--container-max)] mx-auto">
        ...
        <motion.p
          ...
          className="text-text-2 text-lg max-w-2xl">
        ...
        {MODULE_GROUPS.map((group, gi) => (
          <div key={group.title} id={group.slug} className="scroll-mt-24">
            <motion.h2
              ...
              className="flex items-center gap-3 text-xl font-bold text-foreground mb-6">
              <span className="w-1 h-5 bg-amber-500 rounded-full" />
              {group.title}
              <span className="text-text-4 font-normal text-sm font-mono">{group.slugs.length} modules</span>
            </motion.h2>
              ...
              {group.slugs.map((slug, i) => {
                ...
                return (
                  <motion.div key={slug}
                    ...
                    <Link href={`/product/${slug}`} className="group block relative h-60 rounded-2xl overflow-hidden border border-border-dim bg-surface amber-card">
                      ...
                      <div className="absolute inset-0 p-5 flex flex-col justify-between">
                        ...
                        <div>
                          ...
                          <h3 className="text-foreground font-bold text-base mb-1 leading-tight">{m.name}</h3>
                          <p className="text-text-3 text-xs leading-snug line-clamp-2">{m.blurb}</p>
                        </div>
                      </div>
